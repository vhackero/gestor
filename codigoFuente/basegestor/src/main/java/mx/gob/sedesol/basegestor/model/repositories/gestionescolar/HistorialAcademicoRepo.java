...
public class HistorialAcademicoRepo implements IHistorialAcademicoRepo {
	...
	public HistorialAcademicoDTO consultaDatosHistorialAcademico(String id_persona) {
		...
		Object[] dto =  (Object[]) query.getSingleResult();	
		
		regresa.setMatricula(dto[0].toString());
		regresa.setProgramaEducativo(dto[1].toString());
		regresa.setPromedio((BigDecimal) dto[2]);
		regresa.setCreditos((BigDecimal) dto[3]);
		regresa.setAprobadas((BigInteger) dto[4]);
		regresa.setReprobadas((BigInteger) dto[5]);
		regresa.setNopresentadas((BigInteger) dto[6]);
		regresa.setEstatus(dto[7].toString());
		regresa.setNivel(dto[8].toString());
		regresa.setFechaConsulta((Timestamp) dto[9]);
		BigInteger total = ((BigInteger) dto[4]).add((BigInteger) dto[5]).add((BigInteger) dto[6]);
	...
	public List<HistorialAcademicoListaDTO> getParticipanteByActaCerradaYconstancia2(String id_persona) {
		...
		String consulta = "SELECT gp.id as grupo_participante_id, "
				+ "cnp.semestre as 'c/s',\r\n"
