...
public class NuevaBajaRepository implements INuevaBajaRepository {
    ...
    public List<BajaAplicacionDTO> consultarInscripcionesParaBaja(Long idPersona, String nombrePeriodo) {
        String sql = "SELECT DISTINCT ti.id, tpi.proceso_inscripcion_id, per.id_periodo, "
                ...
                + " JOIN tbl_grupos tg ON tg.id_evento = te.id_evento "
                + " JOIN rel_grupo_participante rgp ON rgp.id_grupo = tg.id "
                + " JOIN tbl_ficha_descriptiva_programa fdpEvento "
                + "  ON fdpEvento.id_programa = te.id_programa "
                + " JOIN tbl_ficha_descriptiva_programa fdpInscripcion "
                + "  ON fdpInscripcion.id_programa = ti.idprograma "
                + "  AND fdpInscripcion.id_plan = ti.idplan "
                + " WHERE rgp.id_persona_participante = ti.idpersona "
                + "  AND LOWER(TRIM(fdpEvento.nombre_tentativo)) "
                + "      = LOWER(TRIM(fdpInscripcion.nombre_tentativo)) "
