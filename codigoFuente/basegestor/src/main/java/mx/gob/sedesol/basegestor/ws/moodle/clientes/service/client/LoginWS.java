...
public class LoginWS {
	...
	public String generarAccesoMoodle(String usuario, String password, int idCurso) throws ErrorWS{
		...
		param.setOuth("/local/wsplataforma/tokensisi.php");
		...
		try {
			...
			logger.info(this.parametroWSMoodleDTO.getHost()+"/local/wsplataforma/index_sin_login.php?courseid="+idCurso+"&token="+accessToken);
		} catch (IOException e) {
			e.printStackTrace();
			System.out.println("Detalles del error: " + e.getMessage());
		}
		return this.parametroWSMoodleDTO.getHost()+"/local/wsplataforma/index_sin_login.php?courseid="+idCurso+"&token="+accessToken;
		//return this.parametroWSMoodleDTO.getHost()+"/local/wsplataforma/index_sin_login.php?courseid="+idCurso+"&token="+accessToken;
	}
